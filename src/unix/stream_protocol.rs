...
impl StreamProtocol for UnixStreamProtocol {
    ...
    async fn accept(listener: &mut UnixListener) -> std::result::Result<(UnixStream, SocketAddr), EchoError> {
    ...
    async fn read(stream: &mut UnixStream, buffer: &mut [u8]) -> std::result::Result<usize, EchoError> {
