...
impl EchoServerTrait for UnixStreamEchoServer {
    async fn run(&self) -> Result<()> {
        ...
        info!("Starting Unix domain stream echo server on {}", socket_path.display());
...
impl EchoServerTrait for UnixDatagramEchoServer {
    async fn run(&self) -> Result<()> {
        ...
        info!("Starting Unix domain datagram echo server on {}", socket_path.display());
        
        // Remove existing socket file if it exists
        let _ = std::fs::remove_file(socket_path);
        
        let socket = UnixDatagramProtocol::bind_unix(socket_path).await?;
        info!("Unix domain datagram server bound to {}", socket_path.display());
