...
impl DatagramProtocol for UnixDatagramProtocol {
    ...
    async fn recv_from(socket: &UnixDatagram, buffer: &mut [u8]) -> std::result::Result<(usize, SocketAddr), EchoError> {
    ...
    async fn send_to(socket: &UnixDatagram, data: &[u8], _addr: SocketAddr) -> std::result::Result<usize, EchoError> {
...
impl UnixDatagramExt for UnixDatagramProtocol {
    ...
    async fn connect_unix(_socket_path: &PathBuf) -> std::result::Result<UnixDatagram, EchoError> {
